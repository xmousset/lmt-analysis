...
class SQLiteColumnAdder(QWidget):
    ...
    def load_db(self):
        file_path, _ = QFileDialog.getOpenFileName(
            self, "Open SQLite Database", "", "SQLite Files (*.sqlite *.db)"
        )
        if file_path:
            try:
                ...
                QMessageBox.information(
                    self, "Success", f"Loaded database: {file_path}"
                )
    ...
    def add_column(self):
        ...
        if not table or not col_name:
            QMessageBox.warning(
                self, "Input Error", "Please select a table and enter a column name."
            )
            return
        try:
            cursor = self.conn.cursor()
            ...
            self.conn.commit()
            QMessageBox.information(
                self, "Success", f"Added column '{col_name}' to table '{table}'."
            )
            ...
            if reply == QMessageBox.StandardButton.Yes:
                ...
                if ok:
                    try:
                        ...
                        self.conn.commit()
                        QMessageBox.information(
                            self, "Success", f"Set value for all rows in '{col_name}'."
                        )
                    except Exception as e:
